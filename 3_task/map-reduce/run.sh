...
hadoop jar $HADOOP_STREAMING_JAR_PATH \
        ...
        -input "${HDFS_FULL_TASK_OUTPUT_DIR_PATH}_0" -output "${HDFS_FULL_TASK_OUTPUT_DIR_PATH}_1"

hdfs dfs -rm -r "${HDFS_FULL_TASK_OUTPUT_DIR_PATH}_0"

if [[ $? -gt 0 ]] ; then
    echo "Task 2 failed"
    hdfs dfs -rm -r "${HDFS_FULL_TASK_OUTPUT_DIR_PATH}_1"
    exit 1
fi
...
hadoop jar $HADOOP_STREAMING_JAR_PATH \
        -mapper $TASK_DIR/mapper2.py -reducer $TASK_DIR/reducer2.py \
        -input "${HDFS_FULL_TASK_OUTPUT_DIR_PATH}_1" -output "${HDFS_FULL_TASK_OUTPUT_DIR_PATH}_2"
...
if [[ $? -gt 0 ]] ; then
    echo "Task 3 failed"
    hdfs dfs -rm -r "${HDFS_FULL_TASK_OUTPUT_DIR_PATH}_2"
    exit 1
fi

echo "Task 3 completed successfully, starting task 4"
hadoop jar $HADOOP_STREAMING_JAR_PATH \
        -mapper $TASK_DIR/mapper3.py -reducer $TASK_DIR/reducer3.py \
        -input "${HDFS_FULL_TASK_OUTPUT_DIR_PATH}_2" -output $HDFS_FULL_TASK_OUTPUT_DIR_PATH
